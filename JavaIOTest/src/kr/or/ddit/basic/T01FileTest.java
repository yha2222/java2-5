...
public class T01FileTest {
	public static void main(String[] args) throws IOException {
		...
		//  => 디렉토리와 디렉토리 사이 또는 디렉토리와 파일명 사이의 구분문자는 '\'를 사용하거나 (윈도우 특..'\\')
		// 		'/'를 사용할 수 있다
		...
		File file3 = new File(file2, "test.txt");  // d:/D_Other 안에 있는 text.txt 폴더
		System.out.println(file3.getName() + "의 용량(크기) : " + file3.length()); //해당 파일 용량
		...
		/*
		 	디렉토리(폴더) 만들기
		 	1. mkdir() => File 객체 경로 중 마지막 위치의 디렉토리를 만듦
		 		중간의 경로가 모두 미리 만들어져 있어야 함
		 	2. mkdirs() => 중간 경로가 없으면 중간의 경로도 새롭게 만든 후 마지막 위치의 디렉토리를 만들어줌
		 => 위 두 메서드 모두 만들기를 성공하면 true, 실패하면 flase 반환
		 */
